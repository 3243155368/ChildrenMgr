...
namespace ET.Server
{
    public static class UnitCacheHelper
    {
        public static async ETTask<Unit> GetUnitCache(Scene gateScene, Scene mapScene, long unitId)
        {
            ...
            UnitCache2Other_GetUnit queryUnit = await gateScene.Root().GetComponent<MessageSender>().Call(actorId, message) as UnitCache2Other_GetUnit;
            ...
            for (int i = 0; i < queryUnit.EntityList.Count; i++)
            {
                ...
                EventSystem.Instance.Invoke(SceneType.UnitCache, new AddToBytes() { Unit = unit, Type = type, Bytes = entityBytes });
            }
        ...
        public static void AddOrUpdateUnitAllCache(Unit unit)
        {
            ...
            foreach (Entity entity in unit.Components.Values)
            {
                ...
                byte[] entityBytes = entity.ToBson();
                message.EntityBytes.Add(entityBytes);
                
                EventSystem.Instance.Invoke(SceneType.UnitCache, new AddToBytes() { Unit = unit, Type = entityType });
            }
